...
| **Authentication** | JWT, Refresh Token, Google OAuth 2.0 | Dual-token authentication with HttpOnly cookies and OTP email verification |
...
| **Security** | Helmet, Express Rate Limit, Sharp | Rate limiting, security headers, WebP conversion, and session revocation |
...
│   ├── middlewares/            # Auth guards, rate limiters, validation
...
2. **Configure and start the backend**:
   ...
   Apply database schema and start the server:
   ```bash
   npx prisma db push
   npx prisma generate
   npm run dev
